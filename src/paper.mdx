...
### Taxonomy
Our pipeline starts from an attribute taxonomy $\mathcal{T}$ spanning three environmental axes (scene type, season, weather) and three object-level axes (vehicle count, type, color).
<Figure>
...
We uniformly sample attribute tuples, an LLM composes nadir-view generation prompts, and a text-to-image model (Imagen 3) synthesizes the scenes. 
...
<Tabs defaultValue="taxonomy">
  ...
  <TabsContent value="editing">
    A second image-editing stage rebalances under-represented attributes via image-and-text-to-image edits.
  </TabsContent>
...
<Picture src={modelDiagnosis} alt="Model Diagnosis" />
...
<Picture src={targetedSupplementation} alt="Targeted Supplementation" />
